/**
 * Piper Stub Provider - placeholder
 */
import type { VoiceProvider, ASRAdapter, TTSAdapter } from '../types/index.js';

export class PiperStubProvider implements VoiceProvider {
  asr: ASRAdapter = new StubASR();
  tts: TTSAdapter = new StubTTS();
  
  async initialize() { throw new Error('Piper provider not implemented'); }
  async destroy() {}
  isHealthy() { return false; }
  getStatus() { return { connected: false, provider: 'piper-stub' }; }
